...
const Index = () => {
  ...
  const [currentView, setCurrentView] = useState<"dashboard" | "study">(
    "dashboard"
  );
  ...
  if (currentView === "study") {
    console.log("Rendering StudySession");
    return (
      <StudySession
        onEndSession={() => {
          console.log("StudySession ended, returning to dashboard");
      ...
      <div className="relative z-10 max-w-7xl mx-auto p-6">
        ...
        <div className="text-center mb-5">
          ...
          <p className="text-xl text-foreground/80">
            Your neural mining rig is{" "}
            <span className="text-neon-green font-semibold">online</span> and
            ready to earn
          </p>
        </div>
        ...
        <Card className="p-6 mt-4 bg-card/50 border-2 border-cyber-blue/20 backdrop-blur-sm">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-4">
            {friends.map((friend, i) => (
              <div
                key={i}
                className="p-4 rounded-lg border border-cyber-blue/20 bg-background/30 text-center hover:border-cyber-blue/40 transition-all duration-300"
              >
                <div className="flex items-center justify-center mb-3">
                  <div className="text-2xl mb-2">{friend.avatar}</div>
                  <div
                    className={`w-2 h-2 rounded-full ml-1 ${
                      friend.isOnline ? "bg-neon-green" : "bg-foreground/30"
                    }`}
                  ></div>
                </div>
                <div className="font-semibold text-sm mb-1">{friend.name}</div>
                <div className="text-xs text-cyber-blue mb-1">
                  #{friend.rank}
                </div>
            ...
            const fetchWalletInfo = async () => {
              try {
                ...
                const res = await fetch(
                  "http://127.0.0.1:5000/api/wallet-info",
                  {
                    headers: {
                      Authorization: `Bearer ${token}`,
                    },
