...
export default function StudySession({ onEndSession }: StudySessionProps) {
  ...
  useEffect(() => {
    console.log(
      "StudySession mounted, requesting permissions and starting session..."
    );
    const requestPermissions = async () => {
      try {
        ...
        setCameraPermission(
          cameraPerm.state === "prompt"
            ? "pending"
            : (cameraPerm.state as "granted" | "denied")
        );
        setMicPermission(
          micPerm.state === "prompt"
            ? "pending"
            : (micPerm.state as "granted" | "denied")
        );

        // Always try to get user media if not granted
        if (cameraPerm.state !== "granted" || micPerm.state !== "granted") {
          try {
            ...
            console.error("User denied camera/mic or error:", err);
            setCameraPermission("denied");
            setMicPermission("denied");
          }
        } else {
          // Already granted, set up stream as before
          const stream = await navigator.mediaDevices.getUserMedia({
            video: true,
            audio: true,
          });
  ...
  useEffect(() => {
    ...
    const focusInterval = setInterval(() => {
      const statuses: Array<"focused" | "analyzing" | "distracted"> = [
        "focused",
        "focused",
        "focused",
        "analyzing",
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">
            Focus on your studies while our AI monitors your attention and mines
            crypto.
          </p>
        </div>
